...
| --- | --- |
| Личный кабинет и состояние подписки | Управление пользователями и подписками |
| Тарифы, промокоды и пробный период | Тарифы, сквады, триал и промокоды |
| ЮKassa и CryptoBot | Интеграции и способы оплаты |
| Управление устройствами | Редактор текстов, оформления и функций |
| Поддержка с историей обращений | Тикеты, диагностика и режим аварии |
| Серверы, платежи и реферальная система | Рассылки с предпросмотром и кнопками |
...
Репозиторий приватный, поэтому добавьте SSH-ключ сервера в **GitHub → Repository settings → Deploy keys**. Параметр **Allow write access** включать не нужно.

```bash
ssh-keygen -t ed25519 -C "tgs-bot-vps"
...
Проект распространяется по [закрытой лицензии](LICENSE). Использование и распространение разрешено только владельцу или с его письменного согласия.
