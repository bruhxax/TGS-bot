...
let iconSpritePrefix = '/assets/icons.svg?v=20260914-v120';
...
async function loadIconSprite() {
  try {
    const response = await fetch('/assets/icons.svg?v=20260914-v120', {cache:'force-cache'});
...
const supportSeenKey='tgs_support_seen_v1',userShortcutsKey='tgs_user_shortcuts_v1';
...
function ticketModal() { const topics=[['link','Подключение','Не получается подключиться'],['card','Оплата','Вопрос по оплате'],['clock','Подписка','Вопрос по подписке'],['devices','Устройства','Управление устройствами'],['support','Другое','Другой вопрос']];openModal('Новый тикет', `<form id="ticket-form"><fieldset class="support-topic-picker"><legend>Выберите тему</legend>${topics.map(([ico,label,subject])=>`<button type="button" data-ticket-topic="${esc(subject)}" aria-pressed="false">${icon(ico)}<span>${esc(label)}</span></button>`).join('')}</fieldset>${field('subject','Тема','','text','Можно изменить вручную')}<div class="field"><label for="ticket-description">Описание</label><textarea id="ticket-description" name="description" maxlength="5000" required placeholder="Что случилось?"></textarea></div><label class="support-context-choice"><input type="checkbox" name="include_context" checked><span><strong>Добавить технические данные</strong><small>Статус подписки, срок, платформа и количество устройств помогут ответить быстрее</small></span></label><button class="primary full-width">Отправить</button></form>`); }
...
document.addEventListener('click', async event => {
  ...
  const ticketTopic=event.target.closest('[data-ticket-topic]');if(ticketTopic){const form=ticketTopic.closest('form');form.elements.subject.value=ticketTopic.dataset.ticketTopic;$$('[data-ticket-topic]',form).forEach(button=>button.setAttribute('aria-pressed',String(button===ticketTopic)));form.elements.description.focus();return;}
...
document.addEventListener('change',event=>{if(event.target.matches('[name="shortcut"]')&&event.target.checked){const checked=$$('[name="shortcut"]:checked',event.target.form);if(checked.length>4){event.target.checked=false;toast('Можно выбрать не больше четырёх',true)}}if(event.target.matches('[name="client_all_platforms"]')){const fieldset=$('.platform-picker',event.target.closest('[data-subpage-client]'));if(fieldset)fieldset.disabled=event.target.checked}if(event.target.matches('[name="client_featured"]')&&event.target.checked){$$('[name="client_featured"]',event.target.form).forEach(input=>{if(input!==event.target)input.checked=false})}});
