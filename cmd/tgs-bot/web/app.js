...
function connectPage(){
  ...
  const platformOptions=platforms.map(([id,label])=>`<option value="${id}" ${id===selected?'selected':''}>${esc(label)}</option>`).join('');
  ...
  return `<header class="setup-page-head">${backControl('home')}<h2>${esc(content.connect_title||'Установка')}</h2><label class="setup-platform-select"><span class="sr-only">Устройство</span>${icon('devices')}<select data-setup-platform aria-label="Выберите устройство">${platformOptions}</select>${icon('arrow')}</label></header><p class="setup-lead">${esc(content.connect_hint||'Выберите устройство и приложение — мы проведём вас через три коротких шага.')}</p><div class="setup-client-tabs" role="tablist" aria-label="Приложение">${clientTabs}</div><section class="setup-timeline"><article class="setup-step"><span class="setup-step-marker">${icon('link')}</span><div><h3>${esc(content.connect_install_title||'Установка приложения')}</h3><p>${esc(content.connect_install_hint||'Скачайте клиент для своего устройства и установите приложение.')}</p>${client.install_url?`<a class="setup-inline-action" href="${esc(client.install_url)}" target="_blank" rel="noopener noreferrer">${icon('link')}<span>${esc(content.connect_install_button||'Скачать')} ${esc(client.name)}</span></a>`:''}</div></article><article class="setup-step"><span class="setup-step-marker">${icon('plus')}</span><div><h3>${esc(content.connect_add_title||'Добавление подписки')}</h3><p>${esc(content.connect_add_hint||'Откроется браузер и предложит передать подписку выбранному приложению.')}</p><button type="button" class="primary setup-add-button" data-open-connect-handoff ${url?'':'disabled'}>${icon('plus')}<span>${esc(content.connect_open_button||'Добавить подписку')}</span></button></div></article><article class="setup-step"><span class="setup-step-marker">${icon('check')}</span><div><h3>${esc(content.connect_use_title||'Подключение и использование')}</h3><p>${esc(content.connect_use_hint||'Откройте приложение, выберите сервер и нажмите кнопку подключения.')}</p></div></article></section><section class="setup-qr-section"><h3>${esc(content.connect_qr_title||'QR-код подписки')}</h3><div class="setup-qr-card">${qr}</div></section><button type="button" class="primary setup-copy-button" data-copy="${esc(url)}" ${url?'':'disabled'}>${icon('copy')}<span>${esc(content.connect_copy_button||'Скопировать ссылку')}</span></button>`;
}
...
function contentForms(v) {
  ...
  const connect=`<form data-setting-form="content">${field('connect_title','Заголовок страницы',v.connect_title||'')}${textareaField('connect_hint','Описание страницы',v.connect_hint||'')}<div class="form-grid">${field('connect_install_title','Шаг 1 — заголовок',v.connect_install_title||'')}${field('connect_install_hint','Шаг 1 — описание',v.connect_install_hint||'')}${field('connect_add_title','Шаг 2 — заголовок',v.connect_add_title||'')}${field('connect_add_hint','Шаг 2 — описание',v.connect_add_hint||'')}${field('connect_use_title','Шаг 3 — заголовок',v.connect_use_title||'')}${field('connect_use_hint','Шаг 3 — описание',v.connect_use_hint||'')}${field('connect_qr_title','Заголовок QR-кода',v.connect_qr_title||'')}${field('connect_recommended','Метка рекомендации',v.connect_recommended||'')}${field('connect_open_button','Добавить подписку — кнопка',v.connect_open_button||'')}${field('connect_install_button','Скачать — кнопка',v.connect_install_button||'')}${field('connect_copy_button','Копировать — кнопка',v.connect_copy_button||'')}${field('connect_empty','Нет клиентов',v.connect_empty||'')}</div><button class="primary full-width">Сохранить</button></form>`;
...
document.addEventListener('change',event=>{if(event.target.matches('[data-setup-platform]')){state.cache.setupPlatform=event.target.value;state.cache.setupClient='';render();return}if(event.target.matches('[name="client_all_platforms"]')){const fieldset=$('.platform-picker',event.target.closest('[data-subpage-client]'));if(fieldset)fieldset.disabled=event.target.checked}if(event.target.matches('[name="client_featured"]')&&event.target.checked){$$('[name="client_featured"]',event.target.form).forEach(input=>{if(input!==event.target)input.checked=false})}});
...
async function previewApi(path,options={}) {
  ...
  if(path==='/api/connect/handoff')return {url:'https://example.com/connect/demo',qr_data_url:previewQRDataURL(),expires_at:Math.floor(Date.now()/1000)+900};
