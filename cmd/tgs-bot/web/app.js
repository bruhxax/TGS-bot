...
function connectPage(){
  ...
  return `${detailHead(content.connect_title||'Подключение','home')}<section class="connect-intro"><span>${icon('link')}</span><p>${esc(content.connect_hint||'Выберите устройство и приложение — подписка добавится в клиент одним нажатием.')}</p></section><div class="setup-platforms" role="tablist" aria-label="Устройство">${platforms.map(([id,label])=>`<button type="button" role="tab" aria-selected="${id===selected}" class="${id===selected?'active':''}" data-setup-platform="${id}">${esc(label)}</button>`).join('')}</div><div class="setup-clients">${clients.length?clients.map(client=>`<article class="setup-client"><div class="setup-client-copy"><span class="setup-client-mark">${esc((client.name||'?').slice(0,1).toUpperCase())}</span><span><strong>${esc(client.name)}</strong>${client.featured?`<small>${esc(content.connect_recommended||'Рекомендуем')}</small>`:'<small>Клиент подключения</small>'}</span></div><div class="setup-client-actions">${client.install_url?`<a class="secondary" href="${esc(client.install_url)}" target="_blank" rel="noopener noreferrer">${esc(content.connect_install_button||'Установить')}</a>`:''}<button type="button" class="primary" data-connect-client="${esc(client.id)}" data-connect-platform="${esc(selected)}" ${url?'':'disabled'}>${esc(content.connect_open_button||'Добавить подписку')}</button></div></article>`).join(''):empty('devices',content.connect_empty||'Для этого устройства клиенты пока не добавлены')}</div><button type="button" class="text-copy-button" data-copy="${esc(url)}" ${url?'':'disabled'}>${icon('copy')}<span>${esc(content.connect_copy_button||'Скопировать ссылку')}</span></button>`;
}
...
function contentForms(v) {
  ...
  const connect=`<form data-setting-form="content">${field('connect_title','Заголовок страницы',v.connect_title||'')}${textareaField('connect_hint','Описание',v.connect_hint||'')}<div class="form-grid">${field('connect_recommended','Метка рекомендации',v.connect_recommended||'')}${field('connect_open_button','Добавить подписку — кнопка',v.connect_open_button||'')}${field('connect_install_button','Установить — кнопка',v.connect_install_button||'')}${field('connect_copy_button','Копировать — кнопка',v.connect_copy_button||'')}${field('connect_empty','Нет клиентов',v.connect_empty||'')}</div><button class="primary full-width">Сохранить</button></form>`;
...
document.addEventListener('click', async event => {
  ...
  const buy=event.target.closest('[data-buy]');if(buy){buyModal(buy.dataset.buy);return;}
	const setupPlatform=event.target.closest('[data-setup-platform]');if(setupPlatform){state.cache.setupPlatform=setupPlatform.dataset.setupPlatform;render();return;}
	const connectClient=event.target.closest('[data-connect-client]');if(connectClient){const subscription=state.data.user.subscription.subscription_url||'',client=setupClients(connectClient.dataset.connectPlatform).find(item=>item.id===connectClient.dataset.connectClient);try{const parsed=new URL(subscription);if(!/^https?:$/.test(parsed.protocol)||!client)throw new Error('Ссылка подключения недоступна');const scheme=String(client.scheme||'');if(!scheme.includes('://')||/^(https?|javascript|data|file|blob):/i.test(scheme))throw new Error('Некорректная схема клиента');window.location.assign(`${scheme}${parsed.toString()}`)}catch(e){toast(e.message,true)}return;}
