...
let iconSpritePrefix = '/assets/icons.svg?v=20260914-v130';
...
async function loadIconSprite() {
  try {
    const response = await fetch('/assets/icons.svg?v=20260914-v130', {cache:'force-cache'});
...
function navigate(page) {
  ...
  if (page.startsWith('ticket:')) markSupportTicketSeen(page.split(':')[1]);
  rememberScroll();
  state.page = page;
...
function nav() {
  ...
  const unreadSupport=unseenSupportTickets().length;
  return `<nav class="bottom-nav" data-nav-keys="${items.map(([page]) => page).join(',')}" data-support-unread="${unreadSupport}" style="--nav-count:${items.length};--indicator-x:${activeIndex * 100}%;--nav-width:${items.length * 48 + 16}px" aria-label="Основная навигация"><span class="nav-indicator" aria-hidden="true"></span>${items.map(([page,ico,label]) => `<button class="nav-button ${rootPage === page ? 'active' : ''}" data-nav="${page}" aria-label="${esc(label)}${page==='support'&&unreadSupport?`, новых ответов: ${unreadSupport}`:''}" title="${esc(label)}" ${rootPage === page ? 'aria-current="page"' : ''}>${icon(ico)}${page==='support'&&unreadSupport?`<span class="nav-badge">${Math.min(9,unreadSupport)}</span>`:''}</button>`).join('')}</nav>`;
}
...
function updateNavigation() {
  ...
  const unreadSupport=unseenSupportTickets().length;
  if (!navigation || navigation.dataset.navKeys !== keys || Number(navigation.dataset.supportUnread||0)!==unreadSupport) {
    slot.innerHTML = nav();
...
const supportSeenKey='tgs_support_seen_v1',userShortcutsKey='tgs_user_shortcuts_v1',ticketDraftKey='tgs_ticket_draft_v1',chatDraftsKey='tgs_chat_drafts_v1',adminPrivacyKey='tgs_admin_privacy_v1';
...
function downloadTextFile(filename,text,type='text/plain;charset=utf-8'){const url=URL.createObjectURL(new Blob([text],{type})),link=document.createElement('a');link.href=url;link.download=filename;link.hidden=true;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}
function storedObject(key){try{return JSON.parse(localStorage.getItem(key)||'{}')||{}}catch(_){return{}}}
function supportSeen(){try{return JSON.parse(localStorage.getItem(supportSeenKey)||'{}')||{}}catch(_){return{}}}
function unseenSupportTickets(){if(!state.data||state.data.user?.is_admin)return[];const seen=supportSeen();return(state.data.tickets||[]).filter(ticket=>ticket.status==='answered'&&seen[ticket.id]!==ticket.updated_at)}
function markSupportTicketSeen(id){if(!state.data||state.data.user?.is_admin)return;const ticket=(state.data.tickets||[]).find(item=>item.id===id);if(!ticket)return;const seen=supportSeen();seen[id]=ticket.updated_at;try{localStorage.setItem(supportSeenKey,JSON.stringify(seen))}catch(_){}}
function subscriptionState(){const sub=state.data.user.subscription||{},expires=sub.expires_at?new Date(sub.expires_at):null,remaining=expires?Math.ceil((expires.getTime()-Date.now())/86400000):0,expired=Boolean(expires&&remaining<=0)||['EXPIRED','DISABLED'].includes(sub.status);return{sub,expires,remaining,active:sub.status==='ACTIVE'&&!expired,expired}}
function userAlerts(){if(!state.data)return[];const {sub,remaining,active}=subscriptionState(),alerts=[],unread=unseenSupportTickets(),limit=Number(sub.traffic_limit_bytes||0),used=Number(sub.traffic_used_bytes||0),devices=Number(sub.connected_devices||0),deviceLimit=Number(sub.device_limit||0),payment=(state.data.payments||[]).find(item=>['pending','processing','failed'].includes(item.status));if(unread.length)alerts.push({id:`support-${unread[0].id}`,icon:'support',title:unread.length===1?'Поддержка ответила':`Новых ответов: ${unread.length}`,subtitle:unread[0].subject,target:`ticket:${unread[0].id}`,tone:'attention'});if(active&&remaining<=7)alerts.push({id:'expiry',icon:'clock',title:'Подписка скоро закончится',subtitle:`Осталось ${Math.max(1,remaining)} дн.`,target:'tariffs',tone:'attention'});if(active&&limit>0&&used/limit>=.85)alerts.push({id:'traffic',icon:'traffic',title:'Трафик почти израсходован',subtitle:`Использовано ${Math.min(100,Math.round(used/limit*100))}%`,target:'tariffs',tone:'attention'});if(active&&deviceLimit>0&&devices>=deviceLimit)alerts.push({id:'devices',icon:'devices',title:'Лимит устройств заполнен',subtitle:`Подключено ${devices} из ${deviceLimit}`,target:'more:devices'});if(payment)alerts.push({id:`payment-${payment.id}`,icon:'card',title:payment.status==='failed'?'Оплата не завершена':'Оплата обрабатывается',subtitle:money(payment.amount_rub),target:'more:payments',tone:payment.status==='failed'?'danger':'attention'});if(!active&&state.data.features.trial&&state.data.trial.enabled&&!state.data.user.trial_used)alerts.push({id:'trial',icon:'gift',title:'Доступен пробный период',subtitle:`${state.data.trial.days||3} дня без оплаты`,action:'trial'});return alerts}
function userAlertAction(alert){return alert.action?`data-action="${esc(alert.action)}"`:`data-user-alert-target="${esc(alert.target||'home')}"`}
function openUserInbox(){const alerts=userAlerts(),reminder=state.data.subscription_reminder||{enabled:false,days_before:3};openModal('Для вас',`<section class="user-inbox" aria-live="polite">${alerts.length?alerts.map(alert=>`<button type="button" ${userAlertAction(alert)}><span class="inbox-icon ${esc(alert.tone||'')}">${icon(alert.icon)}</span><span><strong>${esc(alert.title)}</strong><small>${esc(alert.subtitle)}</small></span>${icon('arrow')}</button>`).join(''):`<div class="inbox-clear">${icon('check')}<span><strong>Всё в порядке</strong><small>Важных уведомлений сейчас нет</small></span></div>`}<button type="button" class="inbox-reminder" data-action="subscription-reminder">${icon('calendar')}<span><strong>Напоминание о продлении</strong><small>${reminder.enabled?`За ${reminder.days_before} дн. до окончания`:'Выключено'}</small></span>${icon('arrow')}</button></section>`)}
const shortcutCatalog=[
  {key:'tariffs',icon:'plans',label:'Тарифы',target:'tariffs'},
  {key:'support',icon:'support',label:'Поддержка',target:'support',feature:'support'},
  {key:'devices',icon:'devices',label:'Устройства',target:'more:devices',feature:'devices'},
  {key:'payments',icon:'receipt',label:'Платежи',target:'more:payments',feature:'payments'},
  {key:'referral',icon:'referral',label:'Пригласить',target:'more:referral',feature:'referrals'},
  {key:'reminder',icon:'calendar',label:'Напомнить',action:'subscription-reminder'}
];
function availableShortcuts(){return shortcutCatalog.filter(item=>!item.feature||state.data.features[item.feature])}
function userShortcutKeys(){const allowed=new Set(availableShortcuts().map(item=>item.key));try{const saved=JSON.parse(localStorage.getItem(userShortcutsKey)||'null');if(Array.isArray(saved)){const clean=saved.filter(key=>allowed.has(key)).slice(0,4);if(clean.length)return clean}}catch(_){}return['tariffs','support','devices','payments'].filter(key=>allowed.has(key)).slice(0,4)}
function homeQuickAccess(){const keys=userShortcutKeys(),items=keys.map(key=>availableShortcuts().find(item=>item.key===key)).filter(Boolean);if(!items.length)return'';return `<section class="home-shortcuts" aria-labelledby="home-shortcuts-title"><header><h2 id="home-shortcuts-title">Быстрый доступ</h2><button type="button" data-action="edit-shortcuts" aria-label="Настроить быстрый доступ">${icon('toggles')}</button></header><div>${items.map(item=>`<button type="button" ${item.action?`data-action="${item.action}"`:`data-nav="${item.target}"`}>${icon(item.icon)}<span>${esc(item.label)}</span></button>`).join('')}</div></section>`}
function openShortcutEditor(){const selected=userShortcutKeys();openModal('Быстрый доступ',`<form id="shortcuts-form"><p class="modal-note">Выберите до четырёх действий для главного экрана.</p><div class="shortcut-options">${availableShortcuts().map(item=>`<label><input type="checkbox" name="shortcut" value="${item.key}" ${selected.includes(item.key)?'checked':''}><span>${icon(item.icon)}<strong>${esc(item.label)}</strong></span></label>`).join('')}</div><button class="primary full-width">Сохранить</button></form>`)}
function openReminderModal(){const current=state.data.subscription_reminder||{enabled:false,days_before:3},selected=current.enabled?Number(current.days_before):0;openModal('Напоминание',`<form id="reminder-form"><p class="modal-note">Бот напишет в Telegram до окончания подписки — Mini App открывать не потребуется.</p><fieldset class="reminder-options"><legend>Когда напомнить</legend>${[[1,'За 1 день'],[3,'За 3 дня'],[7,'За неделю'],[14,'За 2 недели'],[0,'Не напоминать']].map(([value,label])=>`<label><input type="radio" name="days_before" value="${value}" ${selected===value?'checked':''}><span>${esc(label)}</span></label>`).join('')}</fieldset><button class="primary full-width">Сохранить</button></form>`)}
function subscriptionPassportText(){const {sub,remaining}=subscriptionState(),brand=state.data.content?.brand||'TGS VPN',limit=Number(sub.traffic_limit_bytes||0),used=Number(sub.traffic_used_bytes||0),devices=Number(sub.connected_devices||0),deviceLimit=Number(sub.device_limit||0);return [`${brand} — подписка`,`Статус: ${statusLabel(sub.status)}`,`Действует до: ${formatDate(sub.expires_at)}`,`Осталось дней: ${Math.max(0,remaining)}`,`Устройства: ${devices} / ${deviceLimit>0?deviceLimit:'∞'}`,`Трафик: ${bytes(used)} / ${limit>0?bytes(limit):'Безлимит'}`].join('\n')}
function openSubscriptionPassport(){const {sub,remaining}=subscriptionState(),limit=Number(sub.traffic_limit_bytes||0),used=Number(sub.traffic_used_bytes||0),deviceLimit=Number(sub.device_limit||0);openModal('Паспорт подписки',`<section class="subscription-passport"><div class="passport-mark">${icon('shield')}<span><strong>${esc(state.data.content?.brand||'TGS VPN')}</strong><small>Цифровая подписка</small></span><span class="status active">${esc(statusLabel(sub.status))}</span></div><dl><div><dt>Действует до</dt><dd>${esc(formatDate(sub.expires_at))}</dd></div><div><dt>Осталось</dt><dd>${Math.max(0,remaining)} дн.</dd></div><div><dt>Устройства</dt><dd>${Number(sub.connected_devices||0)} / ${deviceLimit>0?deviceLimit:'∞'}</dd></div><div><dt>Трафик</dt><dd>${limit?`${esc(bytes(used))} из ${esc(bytes(limit))}`:'Безлимит'}</dd></div></dl><button type="button" class="secondary full-width" data-copy-passport>${icon('copy')}Скопировать данные</button></section>`)}
function calendarDate(value){const date=new Date(value),pad=number=>String(number).padStart(2,'0');return `${date.getFullYear()}${pad(date.getMonth()+1)}${pad(date.getDate())}`}
function calendarEscape(value){return String(value).replace(/\\/g,'\\\\').replace(/([,;])/g,'\\$1').replace(/\n/g,'\\n')}
function addSubscriptionCalendar(){const {sub}=subscriptionState();if(!sub.expires_at){toast('Дата окончания недоступна',true);return}const expires=new Date(sub.expires_at),end=new Date(expires);end.setDate(end.getDate()+1);const brand=state.data.content?.brand||'TGS VPN',stamp=new Date().toISOString().replace(/[-:]/g,'').replace(/\.\d{3}/,'');const ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//TGS-bot//Subscription//RU','CALSCALE:GREGORIAN','BEGIN:VEVENT',`UID:subscription-${state.data.user.id}-${calendarDate(expires)}@tgs-bot`,`DTSTAMP:${stamp}`,`DTSTART;VALUE=DATE:${calendarDate(expires)}`,`DTEND;VALUE=DATE:${calendarDate(end)}`,`SUMMARY:${calendarEscape(`Продлить подписку ${brand}`)}`,`DESCRIPTION:${calendarEscape('Последний день действия подписки. Откройте Mini App и выберите тариф.')}`,'END:VEVENT','END:VCALENDAR'].join('\r\n');downloadTextFile(`subscription-${calendarDate(expires)}.ics`,ics,'text/calendar;charset=utf-8');toast('Событие для календаря готово')}
...
function homePage() {
  ...
  const frame = card => `<div class="home-stage"><div class="home-content"><div class="home-branding"><span aria-hidden="true"></span><div class="home-brand-title">${logo}<p class="home-brand">${esc(content.brand || 'TGS VPN')}</p></div><button type="button" class="home-inbox-button" data-action="user-inbox" aria-label="Уведомления${alertCount?`: ${alertCount}`:''}">${icon('bell')}${alertCount?`<span>${Math.min(9,alertCount)}</span>`:''}</button></div>${card}${homeQuickAccess()}</div></div>`;
  if (!hasSubscription) {
  ...
  return frame(`<section class="hero subscription-active">
    ...
    <div class="subscription-utilities" aria-label="Инструменты подписки"><button type="button" data-action="subscription-passport">${icon('shield')}<span>Паспорт</span></button><button type="button" data-action="subscription-calendar">${icon('calendar')}<span>В календарь</span></button></div>
...
function tariffsPage() {
  ...
  const baseline=Math.max(...tariffs.filter(t=>Number(t.days)>0).map(t=>Number(t.price_rub)/Number(t.days)),0);
  return `<div class="tariffs-stage"><header class="tariffs-intro"><span class="eyebrow">Прозрачное сравнение</span><h1>Выберите подходящий срок</h1><p>Цена за день и реальная выгода рассчитаны автоматически.</p><button type="button" class="tariff-advisor-trigger" data-action="tariff-advisor">${icon('sparkles')}<span><strong>Помощник выбора</strong><small>Подберёт тариф по сроку и устройствам</small></span>${icon('arrow')}</button></header><div class="tariff-list">${tariffs.map(t => { const active=t.id===state.selectedTariffID,daily=Number(t.days)>0?Number(t.price_rub)/Number(t.days):0,saving=baseline>0?Math.max(0,Math.round((1-daily/baseline)*100)):0; return `<button type="button" class="tariff ${active ? 'selected' : ''}" data-select-tariff="${t.id}" aria-pressed="${active}">${t.pinned ? '<span class="tariff-badge">Выгодно</span>' : ''}<h3>${esc(t.name)}</h3><p>${esc(t.description)}</p><div class="tariff-meta"><span>${icon('clock')}${t.days} ${tr('days')}</span><span>${icon('devices')}${Number(t.device_limit) > 0 ? t.device_limit : '∞'} устр.</span><span>${icon('traffic')}${t.traffic_gb ? `${t.traffic_gb} ГБ` : tr('unlimited')}</span></div><div class="tariff-value"><span>${daily.toLocaleString('ru-RU',{maximumFractionDigits:1})} ₽/день</span>${saving?`<strong>Экономия ${saving}%</strong>`:'<small>Базовая стоимость</small>'}</div><div class="tariff-footer"><span class="price">${money(t.price_rub)}</span><span class="tariff-selection">${active ? `${icon('check')}Выбрано` : ''}</span></div></button>`; }).join('')}</div><button class="primary tariff-pay" data-buy="${selected.id}"><span>${tr('buy')} · ${money(selected.price_rub)}</span></button></div>`;
}

function advisedTariff(days,devices){const eligible=(state.data.tariffs||[]).filter(t=>Number(t.days)>=days&&(Number(t.device_limit)===0||Number(t.device_limit)>=devices));const pool=eligible.length?eligible:(state.data.tariffs||[]).filter(t=>Number(t.device_limit)===0||Number(t.device_limit)>=devices);return pool.sort((a,b)=>Math.abs(Number(a.days)-days)-Math.abs(Number(b.days)-days)||Number(a.price_rub)-Number(b.price_rub))[0]||null}
function tariffAdvisorResult(days=90,devices=3){const tariff=advisedTariff(Number(days),Number(devices));if(!tariff)return `<div class="advisor-empty">${icon('alert')}<span>Подходящего тарифа пока нет</span></div>`;const daily=Number(tariff.price_rub)/Math.max(1,Number(tariff.days));return `<button type="button" class="advisor-result" data-advisor-select="${esc(tariff.id)}"><span class="advisor-result-icon">${icon('sparkles')}</span><span><small>Подходит лучше всего</small><strong>${esc(tariff.name)}</strong><em>${tariff.days} дней · ${Number(tariff.device_limit)>0?tariff.device_limit:'∞'} устр. · ${daily.toLocaleString('ru-RU',{maximumFractionDigits:1})} ₽/день</em></span><b>${money(tariff.price_rub)}</b></button>`}
function openTariffAdvisor(){openModal('Помощник выбора',`<form id="tariff-advisor-form"><p class="modal-note">Укажите минимум — помощник найдёт ближайший подходящий тариф.</p><div class="form-grid"><div class="field"><label for="advisor-days">Нужный срок</label><select id="advisor-days" name="days"><option value="30">1 месяц</option><option value="90" selected>3 месяца</option><option value="180">6 месяцев</option><option value="365">1 год</option></select></div><div class="field"><label for="advisor-devices">Устройств</label><select id="advisor-devices" name="devices"><option value="1">1 устройство</option><option value="2">2 устройства</option><option value="3" selected>3 устройства</option><option value="5">5 устройств</option><option value="10">10 устройств</option></select></div></div><div id="tariff-advisor-result" aria-live="polite">${tariffAdvisorResult()}</div></form>`)}
...
async function loadSupportTickets() {
  ...
  try {
    ...
    if(!state.data.user.is_admin&&state.page.startsWith('ticket:'))markSupportTicketSeen(state.page.split(':')[1]);
...
function ticketDraft(){return storedObject(ticketDraftKey)}
function saveTicketDraft(form){if(!form)return;const draft={subject:form.elements.subject?.value||'',description:form.elements.description?.value||''};try{if(draft.subject||draft.description)localStorage.setItem(ticketDraftKey,JSON.stringify(draft));else localStorage.removeItem(ticketDraftKey)}catch(_){}const stateNode=$('.ticket-draft-state',form);if(stateNode)stateNode.textContent=draft.subject||draft.description?'Черновик сохранён':''}
function clearTicketDraft(){try{localStorage.removeItem(ticketDraftKey)}catch(_){}}
function chatDrafts(){return storedObject(chatDraftsKey)}
function saveChatDraft(id,value){const drafts=chatDrafts();if(value)drafts[id]=value;else delete drafts[id];try{localStorage.setItem(chatDraftsKey,JSON.stringify(drafts))}catch(_){}}
function supportCheckItems(){const {sub,active}=subscriptionState(),limit=Number(sub.traffic_limit_bytes||0),used=Number(sub.traffic_used_bytes||0),deviceLimit=Number(sub.device_limit||0),devices=Number(sub.connected_devices||0),payment=(state.data.payments||[]).find(item=>['failed','pending','processing'].includes(item.status));return [
  {ok:active,title:'Подписка',detail:active?`Активна до ${formatDate(sub.expires_at)}`:'Требуется продление'},
  {ok:!limit||used<limit,title:'Трафик',detail:!limit?'Без ограничений':used<limit?`Осталось ${bytes(Math.max(0,limit-used))}`:'Лимит израсходован'},
  {ok:!deviceLimit||devices<deviceLimit,title:'Устройства',detail:!deviceLimit?'Без ограничений':devices<deviceLimit?`Свободно мест: ${deviceLimit-devices}`:'Все места заняты'},
  {ok:!payment,title:'Последняя оплата',detail:!payment?'Нет незавершённых платежей':payment.status==='failed'?'Есть неуспешная оплата':'Платёж ещё обрабатывается'},
  {ok:true,title:'Устройство',detail:setupPlatformMeta.find(([id])=>id===detectedSetupPlatform())?.[1]||'Определено'}
]}
function supportCheckReport(){return ['Результат быстрой проверки',...supportCheckItems().map(item=>`${item.ok?'OK':'Внимание'} — ${item.title}: ${item.detail}`),supportContextText()].join('\n')}
function openSupportCheck(){const items=supportCheckItems(),issues=items.filter(item=>!item.ok).length;state.cache.supportCheckReport=supportCheckReport();openModal('Быстрая проверка',`<section class="support-check"><div class="support-check-summary ${issues?'attention':'ready'}">${icon(issues?'alert':'check')}<span><strong>${issues?`Найдено замечаний: ${issues}`:'Всё выглядит нормально'}</strong><small>${issues?'Проверьте пункты перед подключением':'Если проблема осталась, отправьте результат поддержке'}</small></span></div><div class="support-check-list">${items.map(item=>`<div><span class="check-state ${item.ok?'ok':'warn'}">${icon(item.ok?'check':'alert')}</span><span><strong>${esc(item.title)}</strong><small>${esc(item.detail)}</small></span></div>`).join('')}</div><div class="modal-actions"><button type="button" class="secondary" data-nav-from-modal="connect">${icon('link')}Подключение</button><button type="button" class="primary" data-action="support-check-ticket">${icon('support')}В поддержку</button></div></section>`)}
...
function ticketPage(id) {
  ...
  const restoredDraft=chatDrafts()[id]||'',compose=ticket.status!=='closed'||admin?`<div class="chat-compose-wrap">${restoredDraft?'<small class="chat-draft-restored">Черновик восстановлен</small>':''}<form class="chat-compose" id="chat-form" data-ticket-id="${esc(id)}"><input name="text" maxlength="5000" aria-label="Сообщение" placeholder="Сообщение…" autocomplete="off" value="${esc(restoredDraft)}"><button class="icon-button" aria-label="Отправить">${icon('send')}</button></form></div>`:'<p class="notice">Тикет закрыт</p>';
...
function paymentProviderName(value){return({yookassa:'ЮKassa',cryptobot:'CryptoBot',lava:'LAVA',wata:'WATA',platega:'Platega',freekassa:'FreeKassa',heleket:'Heleket',pally:'Pally'}[String(value||'').toLowerCase()]||value||'Онлайн-оплата')}
function paymentReceiptText(payment){const number=payment.merchant_order_id||String(payment.id||'').slice(0,8),lines=[`Платёж №${number}`,`Сумма: ${money(payment.amount_rub)}`,`Статус: ${statusLabel(payment.status)}`,`Способ: ${paymentProviderName(payment.provider)}`,`Тариф: ${payment.snapshot?.name||'—'}`,`Дата: ${formatStamp(payment.paid_at||payment.created_at)}`];if(payment.promo_code)lines.push(`Промокод: ${payment.promo_code}`);return lines.join('\n')}
function paymentReceiptModal(id){const payment=(state.data.payments||[]).find(item=>String(item.id)===String(id));if(!payment)return;const number=payment.merchant_order_id||String(payment.id||'').slice(0,8),provider=paymentProviderName(payment.provider);openModal(`Платёж №${esc(number)}`,`<section class="payment-receipt"><div class="receipt-amount"><span>${['yookassa','cryptobot','lava','wata','platega','freekassa','heleket','pally'].includes(payment.provider)?paymentLogo(payment.provider):icon('receipt')}</span><strong>${money(payment.amount_rub)}</strong><small class="status ${esc(payment.status)}">${esc(statusLabel(payment.status))}</small></div><dl><div><dt>Тариф</dt><dd>${esc(payment.snapshot?.name||'—')}</dd></div><div><dt>Способ оплаты</dt><dd>${esc(provider)}</dd></div><div><dt>Дата</dt><dd>${esc(formatStamp(payment.paid_at||payment.created_at))}</dd></div>${payment.promo_code?`<div><dt>Промокод</dt><dd>${esc(payment.promo_code)}</dd></div>`:''}</dl><button type="button" class="secondary full-width" data-copy="${esc(paymentReceiptText(payment))}">${icon('copy')}Скопировать данные</button></section>`)}
...
function moreDetail(key) {
  ...
  if (key === 'payments') return `${head}<p class="detail-subtitle">Нажмите на платёж, чтобы открыть подробности</p><div class="list">${state.data.payments?.length ? state.data.payments.map(p => `<button type="button" class="list-row" data-payment-receipt="${esc(p.id)}"><span class="icon-box">${['yookassa','cryptobot','lava','wata','platega','freekassa','heleket','pally'].includes(p.provider)?paymentLogo(p.provider):icon('receipt')}</span><span><strong>${money(p.amount_rub)}</strong><small>${esc(p.snapshot?.name || paymentProviderName(p.provider))} · ${formatDate(p.created_at)}</small></span><span class="status ${p.status}">${statusLabel(p.status)}</span></button>`).join('') : empty('receipt','Платежей пока нет')}</div>`;
  if (key === 'referral') { const ref = state.data.referral,rewarded=Number(ref.rewarded_count||0),days=rewarded*Number(ref.referral_days||0),traffic=rewarded*Number(ref.referral_traffic_gb||0); return `${head}<section class="referral-center"><span class="eyebrow">Ваша программа</span><h2>${Number(ref.count||0)} приглашено</h2><p>Бонус начисляется после выполнения условий реферальной программы.</p><div class="referral-ledger"><div><span>Подтверждено</span><strong>${rewarded}</strong></div><div><span>Начислено дней</span><strong>${days}</strong></div><div><span>Начислено трафика</span><strong>${traffic} ГБ</strong></div></div><div class="code">${esc(ref.link)}</div><div class="referral-actions"><button type="button" class="secondary" data-copy="${esc(ref.link)}">${icon('copy')}Копировать</button><button type="button" class="primary" data-action="share-referral">${icon('share')}Поделиться</button></div></section>`; }
...
function adminPrivacyEnabled(){try{return localStorage.getItem(adminPrivacyKey)==='1'}catch(_){return false}}
function toggleAdminPrivacy(){const next=!adminPrivacyEnabled();try{localStorage.setItem(adminPrivacyKey,next?'1':'0')}catch(_){}return next}
...
function adminHealth(o){const score=adminHealthScore(o),circumference=201.06,offset=(circumference*(100-score)/100).toFixed(2),tone=score>=85?'healthy':score>=65?'attention':'critical',label=score>=85?'Система стабильна':score>=65?'Нужно внимание':'Есть проблемы';return `<button type="button" class="admin-health ${tone}" data-action="admin-health" aria-label="Состояние системы: ${score} из 100. Открыть подробности"><span class="health-ring"><svg viewBox="0 0 76 76" aria-hidden="true"><circle class="health-track" cx="38" cy="38" r="32"></circle><circle class="health-value" cx="38" cy="38" r="32" style="stroke-dashoffset:${offset}"></circle></svg><strong>${score}</strong></span><span class="admin-health-copy"><span class="admin-overline">Состояние системы</span><strong class="admin-health-title">${label}</strong><span class="admin-health-note">Нажмите, чтобы увидеть расчёт показателя.</span></span><span class="health-chevron">${icon('arrow')}</span></button>`}
...
function adminTrend(o){const selected=adminTrendOptions[state.cache.adminTrendMetric]?state.cache.adminTrendMetric:'revenue',meta=adminTrendOptions[selected],trend=o.trends||{},values=(trend[meta.key]||[]).map(Number),labels=trend.labels||[],max=Math.max(1,...values),width=320,height=116,padX=10,padY=13,step=values.length>1?(width-padX*2)/(values.length-1):0,points=values.map((value,index)=>`${(padX+index*step).toFixed(1)},${(height-padY-(value/max)*(height-padY*2)).toFixed(1)}`),total=values.reduce((sum,value)=>sum+value,0),area=points.length?`${padX},${height-padY} ${points.join(' ')} ${padX+(values.length-1)*step},${height-padY}`:'',privateRevenue=adminPrivacyEnabled()&&selected==='revenue',formattedTotal=privateRevenue?'••••':meta.format(total);return `<section class="admin-trend" aria-labelledby="admin-trend-title"><header><div><span class="admin-overline">Последние 7 дней</span><h2 id="admin-trend-title">${esc(meta.label)} · ${esc(formattedTotal)}</h2></div><div class="trend-tabs" role="group" aria-label="Показатель графика">${Object.entries(adminTrendOptions).map(([key,item])=>`<button type="button" data-admin-trend="${key}" aria-pressed="${key===selected}">${esc(item.label)}</button>`).join('')}</div></header><svg class="trend-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(meta.label)} за семь дней"><line x1="${padX}" y1="${height-padY}" x2="${width-padX}" y2="${height-padY}" class="trend-baseline"></line>${area?`<polygon points="${area}" class="trend-area"></polygon><polyline points="${points.join(' ')}" class="trend-line"></polyline>${points.map((point,index)=>{const [cx,cy]=point.split(',');return `<circle cx="${cx}" cy="${cy}" r="3" class="trend-point"><title>${esc(shortTrendDate(labels[index]))}: ${privateRevenue?'скрыто':esc(meta.format(values[index]))}</title></circle>`}).join('')}`:''}</svg><footer><span>${esc(shortTrendDate(labels[0]))}</span><span>${esc(shortTrendDate(labels[labels.length-1]))}</span></footer></section>`}
...
function adminOperationalReport(o){const hidden=adminPrivacyEnabled(),score=adminHealthScore(o);return [`${state.data.content?.brand||'TGS VPN'} — оперативная сводка`,new Intl.DateTimeFormat('ru-RU',{dateStyle:'medium',timeStyle:'short'}).format(new Date()),`Состояние системы: ${score}/100`,`Пользователи: ${o.users} (+${o.new_users_7d||0} за 7 дней)`,`Активные подписки: ${o.active_subscriptions}`,`Открытые тикеты: ${o.open_tickets||0}`,`Диагностика: ${o.diagnostics||0}`,`Ошибки оплат за сутки: ${o.failed_payments_24h||0}`,`Истекают за 3 дня: ${o.expiring_soon||0}`,`Выручка: ${hidden?'скрыта':money((o.revenue_kopecks||0)/100)}`].join('\n')}
async function copyAdminOperationalReport(){const overview=state.cache.overview;if(!overview||overview.error){toast('Сводка ещё загружается',true);return}await copyText(adminOperationalReport(overview));toast('Оперативная сводка скопирована')}
function openAdminHealth(){const o=state.cache.overview;if(!o||o.error)return;const score=adminHealthScore(o),payments=Number(o.payments_30d||0),success=Number(o.successful_payments_30d||0),rate=payments?Math.round(success/payments*100):100,items=[['card','Успешные оплаты',`${rate}% за 30 дней`,'admin:integrations',rate>=95],['support','Открытые тикеты',String(o.open_tickets||0),'support',Number(o.open_tickets||0)===0],['pulse','Диагностика',`${o.diagnostics||0} нерешённых`,'admin:diagnostics',Number(o.diagnostics||0)===0],['clock','Истекают за 3 дня',String(o.expiring_soon||0),'admin:users',Number(o.expiring_soon||0)===0]];openModal('Состояние системы',`<section class="health-details"><div class="health-score-line"><strong>${score}</strong><span>из 100</span></div><p class="modal-note">Оценка учитывает оплаты, обращения, диагностику и ближайшие окончания подписок.</p><div>${items.map(([ico,title,value,target,ok])=>`<button type="button" data-health-target="${target}"><span class="check-state ${ok?'ok':'warn'}">${icon(ico)}</span><span><strong>${esc(title)}</strong><small>${esc(value)}</small></span>${icon('arrow')}</button>`).join('')}</div></section>`)}
...
function adminPage() {
  ...
  const privacy=adminPrivacyEnabled(),command=`<header class="admin-command-head"><div><span class="admin-overline">${esc(state.data.content?.brand||'TGS VPN')} Control</span><h1>Командный центр</h1></div><div class="admin-head-actions"><button type="button" data-action="admin-report" aria-label="Скопировать оперативную сводку">${icon('clipboard')}</button><button type="button" data-action="admin-privacy" aria-label="${privacy?'Показать':'Скрыть'} финансовые показатели" aria-pressed="${privacy}">${icon(privacy?'eye-off':'eye')}</button><button type="button" data-action="admin-command" aria-label="Открыть быстрый поиск">${icon('search')}</button></div></header><button type="button" class="admin-command-trigger" data-action="admin-command">${icon('search')}<span><strong>Найти функцию</strong><small>Настройки, пользователи и операции</small></span><kbd>Ctrl K</kbd></button>`;
  const dashboard=o&&!o.error?`${adminHealth(o)}<div class="admin-metrics">${adminMetric(o.users,'Пользователи',`+${o.new_users_7d||0} за неделю`)}${adminMetric(o.active_subscriptions,'Активные')}${adminMetric(privacy?'••••':money((o.revenue_kopecks||0)/100),'Выручка')}${adminMetric(o.open_tickets||0,'Тикеты','Открытые')}</div>${adminTrend(o)}${adminAttention(o)}`:o?.error?empty('alert',o.error):loadingState();
...
function adminUserMatchesFilter(user,filter){const status=user.subscription?.status,expires=user.subscription?.expires_at?new Date(user.subscription.expires_at):null,soon=expires&&expires>new Date()&&expires<=new Date(Date.now()+7*86400000);return filter==='active'?status==='ACTIVE'&&!user.is_blocked:filter==='expiring'?status==='ACTIVE'&&!user.is_blocked&&soon:filter==='blocked'?Boolean(user.is_blocked):filter==='admins'?Boolean(user.is_admin):true}
function filteredAdminUsers(rows){const filter=state.cache.adminUserFilter||'all';return rows.filter(user=>adminUserMatchesFilter(user,filter))}
function csvCell(value){let text=String(value??'');if(/^[=+\-@]/.test(text))text=`'${text}`;return `"${text.replace(/"/g,'""')}"`}
function exportAdminUsers(){const rows=state.cache['admin:users'];if(!Array.isArray(rows)){toast('Список ещё загружается',true);return}const filtered=filteredAdminUsers(rows),headers=['Telegram ID','Username','Имя','Пользователь Remnawave','Статус','Окончание','Заблокирован','Администратор','Рефералов'],lines=[headers,...filtered.map(user=>[user.telegram_id,user.username||'',user.first_name||'',user.remnawave_username||'',statusLabel(user.subscription?.status),user.subscription?.expires_at?new Date(user.subscription.expires_at).toISOString():'',user.is_blocked?'Да':'Нет',user.is_admin?'Да':'Нет',user.referral_count||0])];downloadTextFile(`users-${new Date().toISOString().slice(0,10)}.csv`,`\uFEFF${lines.map(line=>line.map(csvCell).join(';')).join('\r\n')}`,'text/csv;charset=utf-8');toast(`Экспортировано: ${filtered.length}`)}
function adminUsersView(rows) { if (!Array.isArray(rows)) return empty('user','Ошибка');const filter=state.cache.adminUserFilter||'all',filtered=filteredAdminUsers(rows),filters=[['all','Все'],['active','Активные'],['expiring','Скоро'],['blocked','Блок'],['admins','Админы']];return `<form id="user-search" class="admin-user-search"><label for="admin-user-query">Поиск пользователей</label><div><input id="admin-user-query" name="q" value="${esc(state.cache.adminUserQuery||'')}" placeholder="Имя, username или Telegram ID"><button class="secondary" aria-label="Найти">${icon('search')}</button></div></form><div class="admin-user-filters" role="group" aria-label="Фильтр пользователей">${filters.map(([key,label])=>{const count=rows.filter(user=>adminUserMatchesFilter(user,key)).length;return `<button type="button" data-admin-user-filter="${key}" aria-pressed="${filter===key}"><span>${esc(label)}</span><small>${count}</small></button>`}).join('')}</div><div class="admin-user-summary"><span>Показано <strong>${filtered.length}</strong></span><button type="button" data-action="export-users">${icon('download')}Экспорт CSV</button></div><div class="list">${filtered.length?filtered.map(u=>`<button class="list-row" data-admin-user="${u.id}">${avatar(u,'avatar')}<span><strong>${esc(u.first_name)}</strong><small>${u.username?'@'+esc(u.username):u.telegram_id} · ${statusLabel(u.subscription.status)}${u.is_blocked?' · блок':''}</small></span>${icon('arrow')}</button>`).join(''):empty('user','По выбранному фильтру никого нет')}</div>`; }
...
function ticketModal(prefillSubject='',prefillDescription='') { const topics=[['link','Подключение','Не получается подключиться'],['card','Оплата','Вопрос по оплате'],['clock','Подписка','Вопрос по подписке'],['devices','Устройства','Управление устройствами'],['support','Другое','Другой вопрос']],draft=ticketDraft(),subject=prefillSubject||draft.subject||'',description=prefillDescription||draft.description||'',restored=!prefillSubject&&!prefillDescription&&Boolean(subject||description);openModal('Новый тикет', `<form id="ticket-form"><fieldset class="support-topic-picker"><legend>Выберите тему</legend>${topics.map(([ico,label,topicSubject])=>`<button type="button" data-ticket-topic="${esc(topicSubject)}" aria-pressed="${topicSubject===subject}">${icon(ico)}<span>${esc(label)}</span></button>`).join('')}</fieldset>${field('subject','Тема',subject,'text','Можно изменить вручную')}<div class="field"><label for="ticket-description">Описание</label><textarea id="ticket-description" name="description" maxlength="5000" required placeholder="Что случилось?">${esc(description)}</textarea></div><p class="ticket-draft-state" aria-live="polite">${restored?'Черновик восстановлен':subject||description?'Черновик сохранён':''}</p><label class="support-context-choice"><input type="checkbox" name="include_context" ${prefillDescription?'':'checked'}><span><strong>Добавить технические данные</strong><small>Статус подписки, срок, платформа и количество устройств помогут ответить быстрее</small></span></label><button class="primary full-width">Отправить</button></form>`); }
function supportContextText(){const {sub}=subscriptionState(),platformID=detectedSetupPlatform(),platformLabel=setupPlatformMeta.find(([id])=>id===platformID)?.[1]||platformID,deviceLimit=Number(sub.device_limit)>0?Number(sub.device_limit):'∞';return `\n\n— Технические данные —\nПлатформа: ${platformLabel}\nПодписка: ${statusLabel(sub.status)}\nСрок: ${formatDate(sub.expires_at)}\nУстройства: ${Number(sub.connected_devices||0)} / ${deviceLimit}`}
async function shareReferral(){const ref=state.data.referral||{},url=ref.link||'',text=`Присоединяйтесь к ${state.data.content?.brand||'сервису'}`;if(!url)return;try{if(navigator.share){await navigator.share({title:state.data.content?.brand||'TGS VPN',text,url});return}}catch(error){if(error.name==='AbortError')return}const shareURL=`https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(text)}`;if(tg?.openTelegramLink)tg.openTelegramLink(shareURL);else window.open(shareURL,'_blank','noopener,noreferrer')}
...
document.addEventListener('click', async event => {
  ...
  const alertTarget=event.target.closest('[data-user-alert-target]');if(alertTarget){closeModal(true);navigate(alertTarget.dataset.userAlertTarget);return;}
  const modalNav=event.target.closest('[data-nav-from-modal]');if(modalNav){closeModal(true);navigate(modalNav.dataset.navFromModal);return;}
  const healthTarget=event.target.closest('[data-health-target]');if(healthTarget){closeModal(true);navigate(healthTarget.dataset.healthTarget);return;}
  const advisorChoice=event.target.closest('[data-advisor-select]');if(advisorChoice){state.selectedTariffID=advisorChoice.dataset.advisorSelect;closeModal(true);render();toast('Тариф выбран');return;}
  const userFilter=event.target.closest('[data-admin-user-filter]');if(userFilter){state.cache.adminUserFilter=userFilter.dataset.adminUserFilter;render();return;}
  const receipt=event.target.closest('[data-payment-receipt]');if(receipt){paymentReceiptModal(receipt.dataset.paymentReceipt);return;}
  const ticketTopic=event.target.closest('[data-ticket-topic]');if(ticketTopic){const form=ticketTopic.closest('form');form.elements.subject.value=ticketTopic.dataset.ticketTopic;$$('[data-ticket-topic]',form).forEach(button=>button.setAttribute('aria-pressed',String(button===ticketTopic)));saveTicketDraft(form);form.elements.description.focus();return;}
  ...
  if(action==='new-ticket'){ticketModal();return;} if(action==='new-promo'){promoModal();return;} if(action==='admin-command'){openAdminCommand();return;} if(action==='user-inbox'){openUserInbox();return;} if(action==='edit-shortcuts'){openShortcutEditor();return;} if(action==='subscription-reminder'){openReminderModal();return;} if(action==='subscription-passport'){openSubscriptionPassport();return;} if(action==='subscription-calendar'){addSubscriptionCalendar();return;} if(action==='tariff-advisor'){openTariffAdvisor();return;} if(action==='support-check'){openSupportCheck();return;} if(action==='support-check-ticket'){ticketModal('Проверка подключения',state.cache.supportCheckReport||supportCheckReport());return;} if(action==='share-referral'){await shareReferral();return;} if(action==='admin-privacy'){const enabled=toggleAdminPrivacy();render();toast(enabled?'Финансы скрыты':'Финансы показаны');return;} if(action==='admin-report'){await copyAdminOperationalReport();return;} if(action==='admin-health'){openAdminHealth();return;} if(action==='export-users'){exportAdminUsers();return;}
  ...
  if(event.target.closest('[data-copy-passport]')){await copyText(subscriptionPassportText());toast('Данные подписки скопированы');return;}
  const copy=event.target.closest('[data-copy]');if(copy){await copyText(copy.dataset.copy);toast('Скопировано');return;}
...
document.addEventListener('submit', async event => {
  ...
  try {
    ...
    if(form.id==='shortcuts-form'){const keys=$$('[name="shortcut"]:checked',form).map(input=>input.value).slice(0,4);try{localStorage.setItem(userShortcutsKey,JSON.stringify(keys))}catch(_){}closeModal();render();toast('Быстрый доступ обновлён');return;}
    if(form.id==='reminder-form'){const value=Number(new FormData(form).get('days_before'));const body={enabled:value>0,days_before:value||3},button=$('button',form),old=button.innerHTML;button.disabled=true;button.innerHTML=`${icon('loader')}Сохраняем…`;try{state.data.subscription_reminder=await api('/api/preferences/subscription-reminder',{method:'PUT',body:JSON.stringify(body)});closeModal();render();toast(body.enabled?'Напоминание включено':'Напоминание выключено')}finally{if(document.contains(button)){button.disabled=false;button.innerHTML=old}}return;}
    if(form.id==='ticket-form'){const body=Object.fromEntries(new FormData(form)),includeContext=body.include_context==='on';delete body.include_context;if(includeContext)body.description=`${body.description}${supportContextText()}`;liveMuteSupportUntil=Date.now()+800;const ticket=await api('/api/tickets',{method:'POST',body:JSON.stringify(body)});clearTicketDraft();state.data.tickets.unshift(ticket);closeModal();navigate(`ticket:${ticket.id}`);toast('Тикет создан');return;}
    if(form.id==='chat-form'){const input=form.elements.text,draft=input.value.trim();if(!draft)return;const id=state.page.split(':')[1],currentTicket=visibleTickets().find(t=>t.id===id),oldCount=Math.max(0,currentTicket?.messages?.length||0),button=$('button',form),buttonHTML=button.innerHTML;input.disabled=true;button.disabled=true;button.innerHTML=icon('loader');try{liveMuteSupportUntil=Date.now()+800;const ticket=await api(`/api/tickets/${id}/messages`,{method:'POST',body:JSON.stringify({text:draft})});replaceTicket(ticket);const chat=$('.chat'),messages=ticket.messages||[];if(chat){if(messages.length>=oldCount&&oldCount>0){messages.slice(oldCount).forEach(message=>chat.insertAdjacentHTML('beforeend',ticketMessage(message)))}else{chat.innerHTML=ticketMessages(messages)}chat.scrollTo({top:chat.scrollHeight,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}const status=$('.ticket-state > .status');if(status){status.className=`status ${ticket.status}`;status.textContent=statusLabel(ticket.status)}input.value='';saveChatDraft(id,'');$('.chat-draft-restored')?.remove();}finally{if(document.contains(form)){input.disabled=false;button.disabled=false;button.innerHTML=buttonHTML;input.focus()}}return;}
    ...
    if(form.id==='user-search'){state.cache.adminUserQuery=form.elements.q.value.trim();state.cache['admin:users']=null;await loadAdmin('admin:users',`/api/admin/users?q=${encodeURIComponent(state.cache.adminUserQuery)}`);return;}
...
document.addEventListener('input', event => { if(event.target.matches('input[type="color"]')){event.target.closest('.color-field')?.querySelector('code')?.replaceChildren(event.target.value);const form=event.target.closest('form');if(form)applyTheme({...state.data.theme,...collectForm(form)})}if(event.target.matches('#checkout-promo'))validateCheckoutPromo(event.target);if(event.target.matches('#admin-command-search'))$('#admin-command-results').innerHTML=adminCommandResults(event.target.value);if(event.target.closest('#ticket-form'))saveTicketDraft(event.target.closest('#ticket-form'));if(event.target.matches('#chat-form input'))saveChatDraft(event.target.closest('#chat-form').dataset.ticketId,event.target.value); });
document.addEventListener('change',event=>{if(event.target.matches('[name="shortcut"]')&&event.target.checked){const checked=$$('[name="shortcut"]:checked',event.target.form);if(checked.length>4){event.target.checked=false;toast('Можно выбрать не больше четырёх',true)}}if(event.target.closest('#tariff-advisor-form')){const form=event.target.closest('#tariff-advisor-form');$('#tariff-advisor-result',form).innerHTML=tariffAdvisorResult(Number(form.elements.days.value),Number(form.elements.devices.value))}if(event.target.matches('[name="client_all_platforms"]')){const fieldset=$('.platform-picker',event.target.closest('[data-subpage-client]'));if(fieldset)fieldset.disabled=event.target.checked}if(event.target.matches('[name="client_featured"]')&&event.target.checked){$$('[name="client_featured"]',event.target.form).forEach(input=>{if(input!==event.target)input.checked=false})}});
...
function previewData(){return {
  user:{id:1,telegram_id:6402520205,username:'bruh',remnawave_username:'tgs_6402520205',first_name:'Максим',photo_url:'',is_admin:params.get('role')!=='user',trial_used:false,subscription:{status:'ACTIVE',expires_at:new Date(Date.now()+6*86400000).toISOString(),traffic_limit_bytes:107374182400,traffic_used_bytes:94877025792,device_limit:3,connected_devices:3,subscription_url:'https://example.com/sub'}},
  content:{brand:'TGS VPN',logo_url:'',start_title:'Добро пожаловать',start_text:'',trial_button:'Бесплатный период',connect_button:'Подключиться',renew_button:'Продлить',no_subscription_title:'Подписки нету',subscription_expired_title:'Подписка закончилась',buy_subscription_button:'Купить подписку',traffic_used_label:'Использовано',support_welcome:'Опишите вопрос — поддержка ответит в этом чате.',support_create_button:'Создать тикет',support_tickets_title:'Ваши тикеты',support_admin_title:'Все тикеты',support_admin_hint:'Новые сообщения появляются автоматически',support_empty:'Открытых тикетов нет',connect_title:'Установка',connect_hint:'Выберите устройство и приложение — мы проведём вас через три коротких шага.',connect_recommended:'Рекомендуем',connect_open_button:'Добавить подписку',connect_install_button:'Скачать',connect_copy_button:'Скопировать ссылку',connect_empty:'Для этого устройства клиенты пока не добавлены',connect_install_title:'Установите приложение',connect_install_hint:'Скачайте клиент для своего устройства.',connect_add_title:'Добавьте подписку',connect_add_hint:'Браузер предложит открыть выбранное приложение.',connect_use_title:'Подключитесь',connect_use_hint:'Выберите сервер в приложении и включите подключение.',connect_qr_overline:'Другой способ',connect_qr_title:'Перенести на другое устройство',connect_qr_hint:'Отсканируйте QR-код камерой или сканером внутри VPN-клиента.',subscription_reminder_message:'<b>Подписка скоро закончится</b>\n\nОсталось <b>{days}</b> дн. — до {date}.',emergency_message:'Сервис временно недоступен.'},
...
function previewAdminUsers(){const expires=days=>new Date(Date.now()+days*86400000).toISOString(),base={photo_url:'',trial_used:false,referral_count:0,created_at:new Date(Date.now()-30*86400000).toISOString(),last_seen_at:new Date().toISOString()};return [
  {...base,...state.data.user,is_blocked:false,referral_count:4},
  {...base,id:2,telegram_id:7100000002,username:'anna',remnawave_username:'tgs_anna',first_name:'Анна',is_admin:false,is_blocked:false,subscription:{status:'ACTIVE',expires_at:expires(24),traffic_limit_bytes:214748364800,traffic_used_bytes:48318382080,device_limit:2,connected_devices:1,subscription_url:'https://example.com/sub2'}},
  {...base,id:3,telegram_id:7100000003,username:'oleg',remnawave_username:'tgs_oleg',first_name:'Олег',is_admin:false,is_blocked:true,subscription:{status:'DISABLED',expires_at:expires(12),traffic_limit_bytes:0,traffic_used_bytes:12884901888,device_limit:3,connected_devices:0,subscription_url:'https://example.com/sub3'}},
  {...base,id:4,telegram_id:7100000004,username:'manager',remnawave_username:'tgs_manager',first_name:'Менеджер',is_admin:true,is_blocked:false,subscription:{status:'ACTIVE',expires_at:expires(180),traffic_limit_bytes:0,traffic_used_bytes:32212254720,device_limit:5,connected_devices:2,subscription_url:'https://example.com/sub4'}},
  {...base,id:5,telegram_id:7100000005,username:'dima',remnawave_username:'tgs_dima',first_name:'Дмитрий',is_admin:false,is_blocked:false,subscription:{status:'ACTIVE',expires_at:expires(2),traffic_limit_bytes:107374182400,traffic_used_bytes:85899345920,device_limit:1,connected_devices:1,subscription_url:'https://example.com/sub5'}},
  {...base,id:6,telegram_id:7100000006,username:'maria',remnawave_username:'tgs_maria',first_name:'Мария',is_admin:false,is_blocked:false,subscription:{status:'EXPIRED',expires_at:expires(-9),traffic_limit_bytes:107374182400,traffic_used_bytes:107374182400,device_limit:1,connected_devices:0,subscription_url:'https://example.com/sub6'}}
]}
async function previewApi(path,options={}) {
  await new Promise(resolve=>setTimeout(resolve,80));
  if(path==='/api/preferences/subscription-reminder'){const incoming=JSON.parse(options.body||'{}');state.data.subscription_reminder={enabled:Boolean(incoming.enabled),days_before:Number(incoming.days_before)||3};return state.data.subscription_reminder;}
  ...
  if(path.includes('/users')){const query=new URL(path,location.origin).searchParams.get('q')?.toLocaleLowerCase('ru')||'';return previewAdminUsers().filter(user=>!query||`${user.first_name} ${user.username} ${user.telegram_id} ${user.remnawave_username}`.toLocaleLowerCase('ru').includes(query));}
