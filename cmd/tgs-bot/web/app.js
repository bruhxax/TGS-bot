...
function connectPage(){
  ...
  return `<header class="setup-page-head">${backControl('home')}<h2>${esc(content.connect_title||'Установка')}</h2><div class="setup-platform-control" data-platform-control><button type="button" class="setup-platform-trigger" data-setup-platform-trigger aria-haspopup="listbox" aria-controls="setup-platform-menu" aria-expanded="false">${icon('devices')}<span>${esc(platformLabel)}</span>${icon('arrow')}</button><div class="setup-platform-menu" id="setup-platform-menu" role="listbox" aria-label="Выберите устройство" hidden>${platformOptions}</div></div></header><p class="setup-lead">${esc(content.connect_hint||'Выберите устройство и приложение — мы проведём вас через три коротких шага.')}</p><div class="setup-client-tabs" role="tablist" aria-label="Приложение">${clientTabs}</div><section class="setup-guide"><article class="setup-step"><span class="setup-step-index">01</span><div><h3>${esc(content.connect_install_title||'Установите приложение')}</h3><p>${esc(content.connect_install_hint||'Скачайте клиент для своего устройства.')}</p>${client.install_url?`<a class="setup-inline-action" href="${esc(client.install_url)}" target="_blank" rel="noopener noreferrer"><span>${esc(content.connect_install_button||'Скачать')} ${esc(client.name)}</span>${icon('arrow')}</a>`:''}</div></article><article class="setup-step"><span class="setup-step-index">02</span><div><h3>${esc(content.connect_add_title||'Добавьте подписку')}</h3><p>${esc(content.connect_add_hint||'Браузер предложит открыть выбранное приложение.')}</p><button type="button" class="primary setup-add-button" data-open-connect-handoff ${url?'':'disabled'}>${icon('plus')}<span>${esc(content.connect_open_button||'Добавить подписку')}</span></button></div></article><article class="setup-step"><span class="setup-step-index">03</span><div><h3>${esc(content.connect_use_title||'Подключитесь')}</h3><p>${esc(content.connect_use_hint||'Выберите сервер в приложении и включите подключение.')}</p></div></article></section><section class="setup-transfer"><div class="setup-transfer-copy"><p class="eyebrow">${esc(content.connect_qr_overline||'Другой способ')}</p><h3>${esc(content.connect_qr_title||'Перенести на другое устройство')}</h3><p>${esc(content.connect_qr_hint||'Отсканируйте QR-код камерой или сканером внутри VPN-клиента.')}</p><button type="button" class="secondary setup-copy-button" data-copy="${esc(url)}" ${url?'':'disabled'}>${icon('copy')}<span>${esc(content.connect_copy_button||'Скопировать ссылку')}</span></button></div><div class="setup-qr-card">${qr}</div></section>`;
}
...
function contentForms(v) {
  ...
  const connect=`<form data-setting-form="content">${field('connect_title','Заголовок страницы',v.connect_title||'')}${textareaField('connect_hint','Описание страницы',v.connect_hint||'')}<div class="form-grid">${field('connect_install_title','Шаг 1 — заголовок',v.connect_install_title||'')}${field('connect_install_hint','Шаг 1 — описание',v.connect_install_hint||'')}${field('connect_add_title','Шаг 2 — заголовок',v.connect_add_title||'')}${field('connect_add_hint','Шаг 2 — описание',v.connect_add_hint||'')}${field('connect_use_title','Шаг 3 — заголовок',v.connect_use_title||'')}${field('connect_use_hint','Шаг 3 — описание',v.connect_use_hint||'')}${field('connect_qr_overline','QR — надпись сверху',v.connect_qr_overline||'')}${field('connect_qr_title','QR — заголовок',v.connect_qr_title||'')}${field('connect_qr_hint','QR — описание',v.connect_qr_hint||'')}${field('connect_recommended','Метка рекомендации',v.connect_recommended||'')}${field('connect_open_button','Добавить подписку — кнопка',v.connect_open_button||'')}${field('connect_install_button','Скачать — кнопка',v.connect_install_button||'')}${field('connect_copy_button','Копировать — кнопка',v.connect_copy_button||'')}${field('connect_empty','Нет клиентов',v.connect_empty||'')}</div><button class="primary full-width">Сохранить</button></form>`;
